...
import { 
  ...
  Clock, Flame, ShieldCheck, LogOut, Plus, Scale, RefreshCw, Home, Timer, Droplets, Info, X, Activity, Zap, Star, HelpCircle
} from 'lucide-react';
...
function App() {
  ...
  useEffect(() => {
    try {
      const K = 'KV_V7_STABLE_';
      const p = localStorage.getItem(K + 'user');
      if (p) {
        ...
        setWaterStatus(Number(localStorage.getItem(K + 'water')) || 0);
        setVastenStartTijd(localStorage.getItem(K + 'v_start'));
        setVastenDoel(Number(localStorage.getItem(K + 'v_doel')) || 16);
        setPagina('dashboard');
      }
  ...
  useEffect(() => {
    if (pagina !== 'welkom') {
      ...
      localStorage.setItem(K + 'water', waterStatus.toString());
      localStorage.setItem(K + 'v_doel', vastenDoel.toString());
      if (vastenStartTijd) localStorage.setItem(K + 'v_start', vastenStartTijd);
      else localStorage.removeItem(K + 'v_start');
    }
  }, [profiel, weekPlan, gewichtLog, waterStatus, vastenStartTijd, vastenDoel, pagina]);
  ...
  useEffect(() => {
    ...
    if (vastenStartTijd) {
      interval = setInterval(() => {
        const nu = new Date();
        const start = new Date(vastenStartTijd);
        const doelEinde = new Date(start.getTime() + vastenDoel * 60 * 60 * 1000);
        const verschil = doelEinde - nu;
        if (verschil <= 0) {
          setResterendeTijd("00:00:00");
          clearInterval(interval);
        } else {
          const h = Math.floor(verschil / 3600000);
          const m = Math.floor((verschil % 3600000) / 60000);
          const s = Math.floor((verschil % 60000) / 1000);
          setResterendeTijd(`${h.toString().padStart(2, '0')}:${m.toString().padStart(2, '0')}:${s.toString().padStart(2, '0')}`);
        }
      }, 1000);
    } else { setResterendeTijd(`${vastenDoel}:00:00`); }
    return () => clearInterval(interval);
  }, [vastenStartTijd, vastenDoel]);
  ...
  // --- RENDER RECEPT DETAIL ---
  if (geselecteerdRecept) {
    const r = geselecteerdRecept;
    return (
      <div className="min-h-screen bg-white max-w-md mx-auto flex flex-col font-sans border-x overflow-y-auto pb-32">
        <header className="p-4 border-b sticky top-0 bg-white/95 backdrop-blur-md z-50 flex items-center justify-between text-gray-900">
        ...
        <main>
          <img src={`/recepten/${r?.id}.jpg`} className="h-72 w-full object-cover bg-gray-100" onError={(e) => { e.target.onerror = null; e.target.src = "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=800"; }} />
          <div className="p-8">
            ...
            <div className="bg-green-50 p-4 rounded-2xl border-2 border-green-100 text-green-700 mb-8 flex justify-between items-center shadow-sm">
               ...
               <p className="text-2xl font-black">{r?.macros?.carbs || '0'}g</p>
            </div>
  ...
  return (
    <div className="min-h-screen bg-gray-200 font-sans flex justify-center overflow-x-hidden text-gray-900 leading-tight">
        ...
        <div className="flex-grow overflow-y-auto pb-32">
          {pagina === 'welkom' && (
            <div className="h-screen flex flex-col items-center justify-center p-12 text-center bg-white">
              ...
              <button onClick={() => {localStorage.clear(); window.location.reload();}} className="mt-20 text-gray-300 text-[10px] font-black uppercase underline decoration-gray-200">Volledige Reset</button>
            </div>
          )}

          {pagina === 'onboarding' && (
            <div className="p-10 pt-24 h-screen flex flex-col bg-white">
              <h2 className="text-4xl font-black mb-10 italic uppercase border-b-8 border-[#23a9e4] inline-block self-start">Planning</h2>
              <p className="mb-8 font-bold text-gray-400 uppercase text-xs">Hoe vaak eet je per dag?</p>
          ...
          {pagina === 'dashboard' && (
            <div className="p-6">
              <header className="flex justify-between items-center mb-10 px-2 text-gray-900">
                 <h2 className="text-4xl font-black uppercase italic tracking-tighter border-b-8 border-[#23a9e4]">Vandaag</h2>
                 <div className="flex gap-2">
                  <button onClick={() => genereerPlan(profiel)} className="bg-[#23a9e4]/10 text-[#23a9e4] px-4 py-2 rounded-full font-black text-[10px] border border-[#23a9e4]/20 active:scale-90 transition-all"><center><RefreshCw size={14}/></center> WISSEL MENU</button>
                  <button onClick={() => window.open('/info.html', '_blank')} className="bg-gray-100 text-gray-400 p-2 rounded-full active:scale-90 shadow-sm"><HelpCircle size={24}/></button>
                 </div>
              </header>
              <div onClick={() => setPagina('logboek')} className="bg-gray-900 p-8 rounded-[2.5rem] text-white shadow-2xl mb-10 flex items-center justify-between active:scale-95 transition-all border-b-4 border-[#23a9e4]">
                 <div><p className="text-gray-500 font-black uppercase text-[10px] mb-1 italic">Vasten status</p><p className="text-3xl font-black text-[#23a9e4] tracking-tighter">{resterendeTijd}</p></div>
              ...
              <div className="grid grid-cols-2 gap-6 px-1 text-gray-900">
                <button onClick={() => setPagina('logboek')} className="bg-gray-50 p-8 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col items-center active:scale-95 transition-all text-center text-gray-900">
          ...
          {pagina === 'planner' && <div className="p-8 pb-32 overflow-y-auto h-full bg-white flex flex-col h-full text-gray-900 font-bold"><PaginaHeader titel="Weekplan" /><div className="space-y-10 px-2">{dagenLijst.map((dag) => (<div key={dag} className={`border-l-8 pl-6 ${dag === vandaagNaam ? 'border-[#23a9e4]' : 'border-gray-100'}`}><h3 className={`font-black uppercase italic mb-4 text-2xl ${dag === vandaagNaam ? 'text-[#23a9e4]' : 'text-gray-300'}`}>{dag}</h3><div className="space-y-3">{weekPlan[dag] && Object.entries(weekPlan[dag]).map(([type, r]) => r && (<div key={type} onClick={() => setGeselecteerdRecept(r)} className="flex justify-between items-center bg-gray-50 p-5 rounded-2xl text-lg font-bold shadow-sm active:bg-blue-50 transition-all text-left text-gray-800"><span className="text-gray-400 uppercase text-[10px] w-14 font-black shrink-0">{type}</span><span className="flex-grow truncate px-4 font-bold">{r?.titel}</span><ChevronRight size={20} className="text-gray-200 shrink-0"/></div>))}</div></div>))}</div></div>}

          {pagina === 'logboek' && (
            <div className="p-6 pb-32 flex flex-col h-full overflow-y-auto bg-white text-gray-900 text-center font-bold">
              <PaginaHeader titel="Logboek" />
              <div className="bg-gray-900 p-8 rounded-[3rem] text-white mb-10 border-b-8 border-[#23a9e4] shadow-2xl text-center font-bold">
                  <p className="text-5xl font-black text-[#23a9e4] mb-2">{resterendeTijd}</p>
                  <p className="text-gray-500 font-bold uppercase text-[10px] mb-6 tracking-widest italic text-center">Tijd te gaan</p>
                  <div className="flex gap-2 mb-8 overflow-x-auto pb-2 no-scrollbar justify-center">
                    {[12, 16, 18, 20].map(h => (<button key={h} onClick={() => { setVastenDoel(h); localStorage.setItem('KV_FINAL_STABLE_V6_v_doel', h); }} className={`px-4 py-2 rounded-full font-black text-xs shrink-0 ${vastenDoel === h ? 'bg-[#23a9e4] text-white shadow-lg' : 'bg-white/10 text-gray-400'}`}>{h} uur</button>))}
                  </div>
                  {!vastenStartTijd ? <button onClick={() => { const nu = new Date().toISOString(); setVastenStartTijd(nu); localStorage.setItem('KV_FINAL_STABLE_V6_v_start', nu); }} className="w-full bg-[#23a9e4] py-5 rounded-2xl font-black uppercase shadow-xl active:scale-95 transition-all text-xs">Start Vasten</button> : <button onClick={() => { setVastenStartTijd(null); localStorage.removeItem('KV_FINAL_STABLE_V6_v_start'); }} className="w-full bg-red-500 py-5 rounded-2xl font-black uppercase shadow-xl active:scale-95 transition-all text-xs">Stop Vasten</button>}
                  <div className="mt-8 flex gap-3 text-white-500 text-[10px] italic bg-white/5 p-4 rounded-xl border border-white/5 text-left"><Info size={14} className="text-[#23a9e4] shrink-0"/><p>Water, zwarte koffie en thee mogen tijdens het vasten.</p></div>
              </div>
              <div className="bg-blue-50 p-10 rounded-[3rem] border border-blue-100 mb-10 text-center shadow-inner text-gray-900 font-bold">
                  <h3 className="text-sm font-black uppercase italic mb-6 text-gray-500 flex items-center gap-2 justify-center font-bold text-center"><Droplets size={16} className="text-[#23a9e4]"/> Water drinken</h3>
                  <div className="flex justify-center gap-10 items-center text-gray-900 font-bold text-center">
                      ...
                      <div className="text-center"><p className="text-4xl font-black text-gray-900 leading-none mb-1">{waterStatus}</p><p className="text-gray-400 text-[10px] font-black uppercase tracking-widest text-gray-900 font-bold">Glazen</p></div>
               ...
               <div className="bg-gray-50 p-8 rounded-[3rem] border border-gray-100 flex-grow shadow-inner text-gray-900 font-bold">
          ...
          {pagina === 'profiel' && (
            <div className="p-10 pb-32 h-full text-center bg-white flex flex-col h-full text-gray-900 font-bold">
              <PaginaHeader titel="Beheer" />
              <div className="space-y-6">
                <div className="bg-gray-50 p-10 rounded-[3.5rem] border border-gray-100 mb-12 shadow-inner text-gray-900 font-bold">
                ...
                <button onClick={() => { if(window.confirm("Alles wissen?")) { localStorage.clear(); window.location.reload(); } }} className="w-full bg-red-50 text-red-500 p-8 rounded-[2rem] font-black text-xl active:scale-95 uppercase text-sm text-center font-black">Reset app</button>
                
                <div className="mt-10 bg-gray-100 p-6 rounded-3xl border border-gray-200 shadow-inner text-gray-900 font-bold">
                   <div className="flex items-center gap-3 mb-4 text-center justify-center"><div className="bg-[#23a9e4] p-2 rounded-lg text-white"><ShieldCheck size={20}/></div><h3 className="font-black uppercase text-xs tracking-widest text-gray-700 font-bold text-left">Medische Disclaimer</h3></div>
                   <p className="text-gray-900 text-[10px] leading-relaxed mb-4 font-bold italic text-left">Deze app KetoVoor is door Freddy Sleeuwaert ontwikkeld voor informatieve doeleinden en vormt geen medisch advies. Freddy Sleeuwaert is niet aansprakelijk voor schade voortvloeiend uit het gebruik van deze app.</p>
                   <p className="text-[#23a9e4] font-black text-sm uppercase tracking-tighter italic leading-none">Gemaakt door Freddy Sleeuwaert</p>
                </div>
              </div>
            </div>
          )}
