...
function App() {
  ...
  const getDagenBezig = () => {
    ...
    try {
      // Ondersteunt zowel 07-05-2026 als 07/05/2026
        ...
        <div className="p-6">
          ...
          <div className="space-y-10">
              ...
              {r.tips && r.tips.length > 0 && (
                <div className="bg-amber-50 border-2 border-amber-100 p-6 rounded-3xl italic font-black">
                   ...
                   <ul className="space-y-3 font-black italic">{r.tips.map((tip, i) => <li key={i} className="text-lg text-amber-900 leading-tight">"{tip}"</li>)}</ul>
                </div>
              )}
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gray-50 font-sans flex justify-center text-gray-900 select-none">
      <div className="w-full max-w-md bg-white min-h-screen shadow-2xl flex flex-col relative overflow-hidden italic font-black">
        ...
        <div ref={scrollRef} onScroll={handleScroll} className="flex-grow overflow-y-auto pb-48 px-4 italic font-black font-black italic">
          ...
          {pagina === 'welkom' && (
            <div className="h-screen flex flex-col items-center justify-center text-center px-10">
              ...
              <p className="text-gray-400 uppercase tracking-widest text-sm mb-16 font-black italic font-black">Professional Traject</p>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-2 italic font-black">
                    {['Afvallen', 'Gezondheid', 'Spieropbouw'].map(d => (
                      <button key={d} onClick={() => setProfiel({...profiel, doel: d})} className={`p-4 rounded-2xl text-xl font-black border-2 flex justify-between items-center transition-all italic ${profiel.doel === d ? 'border-blue-600 bg-blue-600 text-white shadow-lg' : 'border-gray-100 text-gray-400 bg-gray-50'}`}><span className="uppercase font-black">{d}</span>{profiel.doel === d && <CheckCircle size={24}/>}</button>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[1, 2, 3].map(n => (
                      <button key={n} onClick={() => setProfiel({...profiel, aantalMaaltijden: n})} className={`p-4 rounded-2xl text-3xl font-black border-2 transition-all italic font-black ${profiel.aantalMaaltijden === n ? 'border-blue-600 bg-blue-600 text-white shadow-lg' : 'bg-gray-50 text-gray-300 font-black'}`}>{n}</button>
                    ))}
                  </div>
                </div>
              </div>
          ...
          {pagina === 'dashboard' && (
            <div className="pt-10 italic">
              <div className="flex justify-between items-center mb-10 italic font-black italic">
              ...
              <div className="space-y-4 font-black italic">
                {weekPlan[vandaagNaam] && Object.entries(weekPlan[vandaagNaam]).map(([type, r]) => (
                  <div key={type} onClick={() => setGeselecteerdRecept(r)} className="bg-white border border-gray-100 rounded-[2.5rem] p-4 flex items-center gap-4 shadow-sm active:scale-95 transition-all border-b-4 italic font-black">
                    <img src={`/recepten/${r.id}.jpg`} className="w-20 h-20 rounded-2xl object-cover bg-gray-50 border shadow-sm italic" />
                    <div className="flex-grow italic font-black">
                      ...
                      <h3 className="text-xl leading-tight mb-2 text-gray-800 uppercase italic font-black italic">{r.titel}</h3>
          ...
          {pagina === 'logboek' && (
            <div className="pt-10 text-left italic">
              ...
              <div className="grid grid-cols-2 gap-4 mb-10 font-black italic italic font-black">
              ...
              <div className="bg-gray-50 p-6 rounded-3xl mb-10 border border-gray-100 font-black italic italic font-black">
                ...
                <div className="flex flex-col gap-4 italic font-black italic">
          ...
          {pagina === 'planner' && (
            <div className="pt-10 italic">
              ...
              <div className="space-y-4 font-black italic">
                {dagenLijst.map(dag => (
                  <div key={dag} className={`p-4 rounded-3xl border-b-2 ${dag === vandaagNaam ? 'bg-blue-50 border-blue-400 shadow-md ring-1 ring-blue-100' : 'bg-white border-gray-100 opacity-60 shadow-sm font-black italic'}`}>
          ...
          {pagina === 'instellingen' && (
            <div className="pt-10 italic font-black italic">
              ...
              <div className="space-y-6 italic font-black italic">
                  <button onClick={() => genereerPlan(profiel)} disabled={isUpdating} className={`w-full py-6 rounded-2xl font-black uppercase text-xl shadow-xl flex items-center justify-center gap-4 italic transition-all ${isUpdating ? 'bg-orange-500 animate-pulse font-black italic' : 'bg-blue-700 text-white font-black italic'}`}>
                    <RefreshCw size={24} className={isUpdating ? 'animate-spin' : ''} />
                    {isUpdating ? 'BEZIG...' : 'PLAN HERBEREKENEN'}
                  </button>
                  <p className="text-xs text-gray-400 uppercase text-center font-black px-4 italic leading-tight italic font-black">Herbereken uw maaltijden voor de volledige komende week op basis van uw nieuw gewicht.</p>
              </div>

              <button onClick={() => { if(window.confirm("Alle gegevens wissen?")) { localStorage.clear(); window.location.reload(); } }} className="w-full py-6 text-red-500 font-black uppercase text-sm border-2 border-red-50 rounded-2xl mt-12 hover:bg-red-50 italic font-black">WISSEN & OPNIEUW BEGINNEN</button>
            </div>
          )}

        </div>

        {/* ZWEVENDE TERUGKNOP - NU CORRECT GEÏMPLEMENTEERD */}
        {showScrollTop && (
          <button onClick={scrollToTop} className="fixed bottom-32 right-6 bg-blue-600 text-white p-4 rounded-full shadow-2xl active:scale-90 transition-all z-[999] border-2 border-white animate-in zoom-in duration-200">
        ...
        <nav className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex justify-around p-2 pb-10 shadow-2xl z-50 rounded-t-3xl font-black italic font-black">
            <button onClick={() => {setPagina('dashboard'); setGeselecteerdRecept(null);}} className={`flex flex-col items-center gap-1 group w-20 py-2 italic font-black ${pagina === 'dashboard' ? 'text-blue-700' : 'text-black opacity-80 font-black'}`}>
            ...
            <button onClick={() => {setPagina('planner'); setGeselecteerdRecept(null);}} className={`flex flex-col items-center gap-1 group w-20 py-2 italic font-black ${pagina === 'planner' ? 'text-blue-700' : 'text-black opacity-80 font-black'}`}>
