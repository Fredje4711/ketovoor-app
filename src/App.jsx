...
function App() {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      if (parsedUser.gewicht && parsedUser.gewicht !== "") setPagina('dashboard');
    }
  }, []);
  ...
  // --- HULP SCHERM (UITGEBREIDE VERSIE) ---
  if (pagina === 'hulp') {
    return (
      <div className="min-h-screen bg-white max-w-md mx-auto flex flex-col font-sans text-gray-900 border-x italic font-black uppercase">
        ...
        <div className="p-6 space-y-10 overflow-y-auto">
          ...
          <section className="space-y-2">
            <h3 className="text-xl font-black text-blue-800 italic uppercase">1. WAT IS DE BEDOELING?</h3>
            <p className="text-base text-gray-600 font-bold italic normal-case">De KetoVoor app is uw persoonlijke assistent. De app rekent exact uit hoeveel u moet eten op basis van uw eigen gewicht en uw doelstelling. Zo krijgt u nooit te veel of te weinig energie binnen.</p>
          </section>

          <section className="space-y-2">
            <h3 className="text-xl font-black text-blue-800 italic uppercase">2. HET DAGMENU</h3>
            <p className="text-base text-gray-600 font-bold italic normal-case">Op het eerste scherm ("VANDAAG") ziet u de maaltijden voor nu. Klik op een gerecht voor de bereiding. Bevalt een voorstel u niet? Klik op "WISSEL" voor een ander gerecht dat ook in uw plan past.</p>
          </section>

          <section className="space-y-2">
            <h3 className="text-xl font-black text-blue-800 italic uppercase">3. RESULTAAT OPSLAAN</h3>
            <p className="text-base text-gray-600 font-bold italic normal-case">Onder de knop "RESULTAAT" kunt u wekelijks uw gewicht invullen. De app onthoudt uw startgewicht en laat u precies zien hoeveel kilo u inmiddels bent verloren.</p>
          </section>

          <section className="space-y-2 bg-blue-50 p-4 rounded-2xl border-4 border-blue-100">
            <h3 className="text-xl font-black text-blue-700 leading-none italic uppercase underline decoration-blue-200">4. ZEER BELANGRIJK!</h3>
            <p className="text-base text-blue-900 font-black italic normal-case font-black">Wanneer u bent afgevallen, moet u het plan laten herberekenen. Ga naar het scherm "BEHEER" en klik op de grote knop "PLAN HERBEREKENEN". De app past dan alle porties aan uw nieuwe gewicht aan.</p>
          </section>

          <section className="space-y-2">
            <h3 className="text-xl font-black text-red-800 italic uppercase">5. OPNIEUW BEGINNEN</h3>
            <p className="text-base text-gray-600 font-bold italic normal-case">Wilt u alle gegevens wissen en weer helemaal vanaf nul beginnen? Gebruik dan de knop onderaan in het "BEHEER" scherm.</p>
          </section>

          <button onClick={() => setPagina('dashboard')} className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black uppercase text-xl shadow-xl mt-10 italic">BEGREPEN</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-white max-w-md mx-auto flex flex-col font-sans text-gray-900 border-x italic font-black">
        <header className="p-4 border-b flex items-center justify-between sticky top-0 bg-white z-50 shadow-sm">
          ...
          <span className="text-[10px] font-black bg-gray-100 px-3 py-1 rounded-full text-gray-400 uppercase italic">DETAILS</span>
        </header>
        <img src={`/recepten/${r.id}.jpg`} className="h-60 w-full object-cover bg-gray-50 border-b shadow-inner italic" />
        <div className="p-6">
          <h2 className="text-2xl font-black mb-6 uppercase leading-tight text-blue-800 border-l-8 border-blue-600 pl-3">{r.titel}</h2>
          <div className="grid grid-cols-4 gap-2 mb-8 text-center font-black bg-blue-50/30 p-3 rounded-2xl border border-blue-100 italic">
             ...
             <div><p className="text-[9px] opacity-40 uppercase font-black">EIWIT</p><p className="text-base text-blue-700 font-black">{r.macros.eiwit}g</p></div>
             <div><p className="text-[9px] opacity-40 uppercase font-black">VET</p><p className="text-base text-yellow-600 font-black">{r.macros.vet}g</p></div>
          ...
          <div className="space-y-8 font-black">
              ...
              <div className="space-y-1">{r.ingredienten.map((ing, i) => <div key={i} className="flex justify-between border-b border-gray-50 py-2 text-base font-bold text-gray-700"><span>{ing.item}</span><span className="text-blue-700 font-black italic">{ing.hoeveelheid} {ing.eenheid}</span></div>)}</div>
              <h3 className="font-black uppercase text-sm border-b-2 border-blue-600 pb-1 inline-block italic font-black">BEREIDING</h3>
              <div className="space-y-6">{r.instructies.map((ins, i) => <div key={i} className="flex gap-4 text-base leading-tight text-gray-700 font-medium border-l-4 border-blue-100 pl-4 italic"><p>{ins}</p></div>)}</div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans flex justify-center text-gray-900 select-none font-black italic uppercase">
      <div className="w-full max-w-md bg-white min-h-screen shadow-2xl flex flex-col relative overflow-hidden">
        <div ref={scrollRef} key={pagina} className="flex-grow overflow-y-auto pb-40 px-4">
          ...
          {pagina === 'welkom' && (
            <div className="h-screen flex flex-col items-center justify-center text-center px-10">
              ...
              <button onClick={() => setPagina('onboarding')} className="w-full bg-blue-600 text-white py-6 rounded-3xl font-black text-2xl shadow-xl active:scale-95 transition-all uppercase">START</button>
            </div>
          )}
          ...
          {pagina === 'onboarding' && (
            <div className="pt-6 flex flex-col h-full italic">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-2 mt-2 font-black italic">
                  {[1, 2, 3].map(n => (
                    <button key={n} onClick={() => setProfiel({...profiel, aantalMaaltijden: n})} className={`p-4 rounded-xl text-2xl border-2 transition-all italic ${profiel.aantalMaaltijden === n ? 'border-blue-600 bg-blue-600 text-white shadow-md' : 'bg-gray-50 text-gray-300 font-black'}`}>{n}</button>
                  ))}
                </div>
              </div>
          ...
          {pagina === 'dashboard' && (
            <div className="pt-6 italic font-black">
              <div className="flex flex-col gap-4 mb-8">
                ...
                <button onClick={() => setPagina('hulp')} className="w-full bg-white text-blue-600 py-3 rounded-xl text-base font-black border-2 border-blue-600 flex items-center justify-center gap-2 mb-2 italic shadow-md uppercase">HULP & UITLEG <HelpCircle size={20} strokeWidth={3}/></button>
              </div>
                ...
                {weekPlan[vandaagNaam] && Object.entries(weekPlan[vandaagNaam]).map(([type, r]) => (
                  <div key={type} onClick={() => setGeselecteerdRecept(r)} className="bg-white border-2 border-gray-100 rounded-[2.5rem] p-4 flex items-center gap-4 shadow-sm active:scale-95 border-b-8">
                    ...
                    <div className="flex-grow italic">
                      ...
                      <div className="flex gap-4 text-[10px] font-black uppercase italic italic font-black"><span className="text-orange-500 font-black italic">{r.macros.kcal} KCAL</span><span className="text-blue-600 font-black">{r.macros.eiwit}G EIWIT</span></div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="pt-6 text-left italic font-black font-black">
              ...
              <div className="space-y-2 font-black italic">
                {[...gewichtLog].reverse().map((log, i) => (
                  <div key={i} className="flex justify-between items-center px-6 py-4 bg-white border border-gray-100 rounded-xl shadow-sm italic font-black">
                    <span className="text-gray-400 text-[10px] font-black uppercase italic font-black">{log.datum}</span>
                    <span className="text-2xl text-gray-800 font-black italic italic">{log.kg} KG</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {pagina === 'planner' && (
            <div className="pt-6 italic font-black font-black">
              <h2 className="text-3xl font-black uppercase border-b-4 border-blue-600 inline-block mb-6 italic">WEEKPLAN</h2>
              <div className="space-y-3 font-black">
                {dagenLijst.map(dag => (
                      ...
                      {weekPlan[dag] && Object.entries(weekPlan[dag]).map(([type, r]) => (
                        <div key={type} onClick={() => setGeselecteerdRecept(r)} className="flex justify-between items-center bg-white px-4 py-3 rounded-xl shadow-sm active:bg-blue-50 border border-gray-50 italic">
                          <div className="flex items-center gap-4 italic font-black">
                             ...
                             <span className="text-gray-800 uppercase italic text-base font-black">{r.titel}</span>
                          </div>
          ...
          {pagina === 'instellingen' && (
            <div className="pt-6 italic font-black font-black">
              ...
              <div className="bg-blue-50 p-6 rounded-2xl mb-6 border border-blue-100 font-black italic">
                <p className="text-[10px] text-blue-800 uppercase mb-4 border-b-2 border-blue-200 pb-2 text-center font-black italic">UW PROFIEL</p>
                <div className="space-y-2 italic font-black">
                  <div className="flex justify-between items-center text-xl font-black italic"><span>GEWICHT</span> <span className="text-blue-700 italic font-black">{profiel.gewicht} KG</span></div>
        ...
        {/* --- CUSTOM MODALS --- */}
        {showErrorModal && (<div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[9999] flex items-center justify-center p-8 uppercase italic font-black"><div className="bg-white w-full rounded-[2.5rem] p-8 text-center shadow-2xl border-t-[10px] border-red-600 italic"><XCircle size={80} className="text-red-600 mx-auto mb-4" /><h3 className="text-3xl font-black mb-2 italic">GEWICHT VERGETEN</h3><p className="text-base text-gray-600 mb-8 font-black leading-tight">VUL AUB EERST UW GEWICHT IN VOORDAT U VERDER KUNT GAAN.</p><button onClick={() => setShowErrorModal(false)} className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black text-xl uppercase italic italic font-black">IK GA HET INVULLEN</button></div></div>)}
        {showResetModal && (<div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[9999] flex items-center justify-center p-8 uppercase italic font-black"><div className="bg-white w-full rounded-[3.5rem] p-10 text-center shadow-2xl border-t-[15px] border-orange-500 font-black"><AlertTriangle size={100} className="text-orange-500 mx-auto mb-6 animate-pulse" /><h3 className="text-4xl font-black mb-4 italic font-black">ALLES WISSEN?</h3><p className="text-xl text-gray-600 mb-12 font-black italic uppercase italic italic font-black">WEET U ZEKER DAT U ALLES WILT WISSEN EN OPNIEUW WILT BEGINNEN?</p><div className="flex flex-col gap-4 font-black italic"><button onClick={fullReset} className="w-full bg-red-600 text-white py-6 rounded-3xl font-black text-2xl uppercase font-black italic">JA, WIS ALLES</button><button onClick={() => setShowResetModal(false)} className="w-full bg-gray-100 text-gray-600 py-4 rounded-3xl font-black text-lg uppercase italic font-black">NEE, STOP</button></div></div></div>)}
        {showSuccessSaved && (<div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[9999] flex items-center justify-center p-8 uppercase italic font-black font-black"><div className="bg-white w-full rounded-[3.5rem] p-10 text-center shadow-2xl border-t-[15px] border-blue-600 font-black font-black"><CheckCircle size={100} className="text-blue-600 mx-auto mb-6 font-black" /><h3 className="text-3xl font-black mb-4 font-black italic italic uppercase">OPGESLAGEN!</h3><p className="text-lg text-gray-600 mb-10 font-black italic italic">UW NIEUWE GEWICHT IS GEREGISTREERD.</p><button onClick={() => { setShowSuccessSaved(false); setPagina('instellingen'); }} className="w-full bg-blue-600 text-white py-6 rounded-3xl font-black text-2xl uppercase italic italic font-black">PAS MIJN PLAN NU AAN</button></div></div>)}

        {/* --- NAVIGATIE ONDERAAN --- */}
        <nav className="absolute bottom-0 left-0 right-0 bg-white border-t-4 border-gray-50 flex justify-around p-2 pb-12 shadow-2xl z-50 rounded-t-3xl font-black italic">
            <button onClick={() => gaNaar('dashboard')} className={`flex flex-col items-center gap-1 group w-20 py-2 italic font-black ${pagina === 'dashboard' ? 'text-blue-700 font-black italic' : 'text-black opacity-100'}`}><div className={`p-2 rounded-xl ${pagina === 'dashboard' ? 'bg-blue-50' : 'bg-transparent'} transition-all font-black`}><Utensils size={28} strokeWidth={2.5} /></div><span className="text-[10px] uppercase font-black italic">VANDAAG</span></button>
            <button onClick={() => gaNaar('planner')} className={`flex flex-col items-center gap-1 group w-20 py-2 italic font-black ${pagina === 'planner' ? 'text-blue-700 font-black italic' : 'text-black opacity-100'}`}><div className={`p-2 rounded-xl ${pagina === 'planner' ? 'bg-blue-50' : 'bg-transparent'} transition-all font-black`}><Calendar size={28} strokeWidth={2.5} /></div><span className="text-[10px] uppercase font-black italic font-black font-black">WEEKPLAN</span></button>
            <button onClick={() => gaNaar('logboek')} className={`flex flex-col items-center gap-1 group w-20 py-2 italic font-black ${pagina === 'logboek' ? 'text-blue-700' : 'text-black opacity-100'}`}><div className={`p-2 rounded-xl ${pagina === 'logboek' ? 'bg-blue-50' : 'bg-transparent'} transition-all font-black`}><Scale size={28} strokeWidth={2.5} /></div><span className="text-[10px] uppercase font-black italic font-black">RESULTAAT</span></button>
            <button onClick={() => gaNaar('instellingen')} className={`flex flex-col items-center gap-1 group w-20 py-2 italic font-black ${pagina === 'instellingen' ? 'text-blue-700' : 'text-black opacity-100'}`}><div className={`p-2 rounded-xl ${pagina === 'instellingen' ? 'bg-blue-50' : 'bg-transparent'} transition-all font-black`}><User size={28} strokeWidth={2.5} /></div><span className="text-[10px] uppercase font-black italic font-black font-black">BEHEER</span></button>
        </nav>
