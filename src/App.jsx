...
import { 
  ...
  Target, History, TrendingDown, CheckCircle, ArrowUp, Star, AlertCircle, HelpCircle
} from 'lucide-react';
...
function App() {
  ...
  const K = 'KV_V24_FINAL_';

  // Forceer scroll naar boven bij elke paginawissel
  useEffect(() => {
    window.scrollTo(0, 0);
    ...
    return (
      <div className="min-h-screen bg-white max-w-md mx-auto flex flex-col font-sans text-gray-900 border-x italic font-black">
        <header className="p-5 border-b flex items-center justify-between sticky top-0 bg-white z-50">
          <button onClick={() => setPagina('dashboard')} className="flex items-center gap-1 text-blue-600 font-bold uppercase text-sm">← TERUG</button>
          <span className="text-xs font-black bg-blue-600 text-white px-4 py-1 rounded-full uppercase">HULP</span>
        </header>
        <div className="p-8 space-y-10">
          <h2 className="text-3xl uppercase border-b-4 border-blue-600 inline-block mb-2 font-black italic">HOE WERKT HET?</h2>
          <div className="space-y-8">
            <div className="flex gap-4"><div className="bg-blue-600 text-white w-8 h-8 rounded-full flex items-center justify-center shrink-0 font-black italic">1</div><p className="text-lg leading-tight font-bold italic text-gray-700">Deze app berekent uw maaltijden op basis van uw huidige gewicht.</p></div>
            <div className="flex gap-4"><div className="bg-blue-600 text-white w-8 h-8 rounded-full flex items-center justify-center shrink-0 font-black italic">2</div><p className="text-lg leading-tight font-bold italic text-gray-700">Wilt u iets anders eten? Klik in het menu op "WISSEL GERECHTEN".</p></div>
            <div className="flex gap-4 border-l-4 border-orange-400 pl-4 py-2 bg-orange-50"><p className="text-orange-800 font-black uppercase text-sm italic italic">BELANGRIJK: Na gewichtsverlies, klik in het "BEHEER" scherm op "PLAN HERBEREKENEN".</p></div>
          </div>
          <button onClick={() => setPagina('dashboard')} className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black uppercase text-xl shadow-lg mt-10">IK BEGRIJP HET</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-white max-w-md mx-auto flex flex-col font-sans text-gray-900 border-x">
        <header className="p-4 border-b flex items-center justify-between sticky top-0 bg-white z-50">
          <button onClick={() => setGeselecteerdRecept(null)} className="text-blue-600 font-black text-sm uppercase italic">← TERUG</button>
        ...
        <div className="p-6 font-black italic">
          ...
          <div className="grid grid-cols-4 gap-2 mb-8 text-center font-black bg-blue-50/30 p-3 rounded-2xl border border-blue-100 italic">
             ...
             <div><p className="text-[9px] opacity-40 uppercase italic">EIWIT</p><p className="text-base text-blue-700">{r.macros.eiwit}g</p></div>
             <div><p className="text-[9px] opacity-40 uppercase italic">VET</p><p className="text-base text-yellow-600">{r.macros.vet}g</p></div>
             <div><p className="text-[9px] opacity-40 uppercase italic">KOOLH.</p><p className="text-base text-green-600">{r.macros.carbs}g</p></div>
          </div>
          <div className="space-y-8">
              <h3 className="font-black uppercase text-sm border-b-2 border-blue-600 pb-1 inline-block italic">INGRÉDIËNTEN</h3>
              <div className="space-y-1">{r.ingredienten.map((ing, i) => <div key={i} className="flex justify-between border-b border-gray-50 py-2 text-lg italic"><span>{ing.item}</span><span className="text-blue-700 font-black uppercase text-base">{ing.hoeveelheid} {ing.eenheid}</span></div>)}</div>
              <h3 className="font-black uppercase text-sm border-b-2 border-blue-600 pb-1 inline-block italic">BEREIDING</h3>
              <div className="space-y-6">{r.instructies.map((ins, i) => <div key={i} className="flex gap-4 text-lg leading-tight text-gray-700 italic border-l-4 border-blue-100 pl-4 italic"><p>{ins}</p></div>)}</div>
              {r.tips && r.tips.length > 0 && (
                <div className="bg-amber-50 border-2 border-amber-100 p-6 rounded-3xl italic mt-6">
    ...
    <div className="min-h-screen bg-gray-50 font-sans flex justify-center text-gray-900 select-none">
      <div className="w-full max-w-md bg-white min-h-screen shadow-2xl flex flex-col relative overflow-hidden italic font-black">
        ...
        <div ref={scrollRef} key={pagina} className="flex-grow overflow-y-auto pb-44 px-4 italic font-black">
          ...
          {pagina === 'welkom' && (
            <div className="h-screen flex flex-col items-center justify-center text-center px-10 italic">
              ...
              <h1 className="text-6xl font-black italic text-blue-600 mb-2 uppercase leading-none italic font-black">KETO<br/>VOOR</h1>
              <p className="text-gray-400 uppercase tracking-widest text-sm mb-16 italic font-black">Personalised Traject</p>
              <button onClick={() => setPagina('onboarding')} className="w-full bg-blue-600 text-white py-6 rounded-2xl font-black text-2xl shadow-xl active:scale-95 transition-all uppercase italic font-black">START</button>
            </div>
          )}
            ...
            <div className="pt-10 flex flex-col h-full italic">
              ...
              <div className="space-y-10 font-black italic">
                <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100 text-center italic">
                  ...
                  <input type="text" inputMode="decimal" value={profiel.gewicht} placeholder="TYP UW GEWICHT" onChange={(e) => setProfiel({...profiel, gewicht: e.target.value})} className="w-full bg-white border-2 border-blue-100 p-4 rounded-2xl text-center font-black text-5xl text-blue-600 outline-none placeholder:text-blue-100 placeholder:text-lg italic" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[1, 2, 3].map(n => (
                      <button key={n} onClick={() => setProfiel({...profiel, aantalMaaltijden: n})} className={`p-5 rounded-2xl text-4xl border-2 transition-all italic ${profiel.aantalMaaltijden === n ? 'border-blue-600 bg-blue-600 text-white' : 'bg-gray-50 text-gray-300'}`}>{n}</button>
                    ))}
                  </div>
                </div>
              </div>
            ...
            <div className="pt-10 italic">
              ...
              <div className="space-y-4">
                {weekPlan[vandaagNaam] && Object.entries(weekPlan[vandaagNaam]).map(([type, r]) => (
                  <div key={type} onClick={() => setGeselecteerdRecept(r)} className="bg-white border border-gray-100 rounded-[2.5rem] p-4 flex items-center gap-4 shadow-sm active:scale-95 transition-all border-b-4 italic">
                    <img src={`/recepten/${r.id}.jpg`} className="w-20 h-20 rounded-2xl object-cover bg-gray-50 border italic" />
                    <div className="flex-grow italic">
                      <p className="text-[10px] uppercase text-blue-700 font-black opacity-40 italic">{type}</p>
                      <h3 className="text-xl leading-tight mb-2 text-gray-800 uppercase italic font-black italic tracking-tight">{r.titel}</h3>
                      <div className="flex gap-4 text-[10px] font-black text-gray-400 uppercase italic"><span className="text-orange-500">{r.macros.kcal} KCAL</span><span className="text-blue-600 font-black italic">{r.macros.eiwit}G EIWIT</span></div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {pagina === 'logboek' && (
            <div className="pt-10 text-left italic font-black">
              ...
              <div className="grid grid-cols-2 gap-4 mb-10 italic">
                 ...
                 <div className="bg-gray-100 p-6 rounded-3xl text-gray-600 text-center border-b-4 border-gray-200 shadow-inner italic">
                    <p className="text-[10px] uppercase opacity-40 mb-1 italic font-black italic">DAGEN</p>
                    <p className="text-4xl font-black italic italic">{getDagenBezig()}</p>
                 </div>
              </div>

              <div className="bg-gray-50 p-6 rounded-3xl mb-10 border border-gray-100 font-black italic italic">
                ...
                <div className="flex flex-col gap-4 italic italic">
                  <div className="flex items-center justify-center bg-white p-4 rounded-2xl border-2 border-gray-100 italic font-black">
                    <input type="text" inputMode="decimal" value={nieuwGewicht} onChange={(e) => setNieuwGewicht(e.target.value)} placeholder="00" className="w-full bg-transparent text-center font-black text-5xl text-blue-700 outline-none italic" />
              ...
              {laatsteGewichtUpdate && (
                <div className="bg-orange-50 border-8 border-orange-400 p-8 rounded-[3rem] mb-10 animate-bounce text-center italic">
                    <AlertCircle size={56} className="mx-auto text-orange-600 mb-4 font-black" />
                    <p className="text-2xl font-black text-orange-900 uppercase leading-none mb-6 italic">GEWICHT IS AANGEPAST!</p>
                    <button onClick={() => genereerPlan(profiel)} className="bg-orange-600 text-white w-full py-6 rounded-2xl font-black text-xl uppercase shadow-2xl">PLAN NU BIJWERKEN →</button>
                </div>
              )}
              ...
              <div className="space-y-2 font-black italic">
                {[...gewichtLog].reverse().map((log, i) => (
                  <div key={i} className="flex justify-between items-center p-5 bg-white border border-gray-100 rounded-2xl shadow-sm font-black italic">
                    <span className="text-gray-400 text-xs font-black uppercase italic font-black">{log.datum}</span>
          ...
          {pagina === 'planner' && (
            <div className="pt-10 italic">
              ...
              <div className="space-y-2 italic font-black">
                {dagenLijst.map(dag => (
                  <div key={dag} className={`p-3 rounded-2xl border-b-2 font-black italic ${dag === vandaagNaam ? 'bg-blue-50 border-blue-400 shadow-md ring-1 ring-blue-100' : 'bg-white border-gray-100 opacity-60 shadow-sm'}`}>
                    <h3 className={`uppercase text-sm mb-2 font-black italic ${dag === vandaagNaam ? 'text-blue-700 underline decoration-blue-200' : 'text-gray-500'}`}>{dag} {dag === vandaagNaam && "• VANDAAG"}</h3>
                      ...
                      {weekPlan[dag] && Object.entries(weekPlan[dag]).map(([type, r]) => (
                        <div key={type} onClick={() => setGeselecteerdRecept(r)} className="flex justify-between items-center bg-white px-4 py-2 rounded-xl active:bg-blue-50 border border-gray-50 italic">
                          <div className="flex items-center gap-4 italic font-black">
                             ...
                             <span className="text-gray-800 font-black text-sm uppercase italic tracking-tighter italic">{r.titel}</span>
                          </div>
                          <ChevronRight size={18} className="text-gray-200 italic" />
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="pt-10 italic font-black italic">
              ...
              <div className="bg-blue-50 p-6 rounded-3xl mb-8 border border-blue-100 font-black italic font-black">
                ...
                <div className="space-y-4 italic font-black">
                  ...
                  <div className="flex justify-between items-center text-xl italic font-black italic"><span>RITME:</span> <span className="text-blue-700 font-black italic">{profiel.aantalMaaltijden} P/D</span></div>
                </div>
              </div>

              <div className="space-y-6 font-black italic font-black">
                  <button onClick={() => genereerPlan(profiel)} disabled={isUpdating} className={`w-full py-6 rounded-2xl font-black uppercase text-xl shadow-xl flex items-center justify-center gap-4 italic transition-all ${isUpdating ? 'bg-orange-500 animate-pulse font-black' : 'bg-blue-700 text-white font-black'}`}>
        ...
        <nav className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex justify-around p-2 pb-10 shadow-2xl z-50 rounded-t-3xl font-black italic font-black">
            <button onClick={() => {setPagina('dashboard'); setGeselecteerdRecept(null);}} className={`flex flex-col items-center gap-1 group w-20 py-2 italic font-black ${pagina === 'dashboard' ? 'text-blue-700 font-black' : 'text-black opacity-80 font-black'}`}>
            ...
            <button onClick={() => {setPagina('planner'); setGeselecteerdRecept(null);}} className={`flex flex-col items-center gap-1 group w-20 py-2 italic font-black ${pagina === 'planner' ? 'text-blue-700 font-black' : 'text-black opacity-80 font-black'}`}>
            ...
            <button onClick={() => {setPagina('logboek'); setGeselecteerdRecept(null);}} className={`flex flex-col items-center gap-1 group w-20 py-2 italic font-black ${pagina === 'logboek' ? 'text-blue-700' : 'text-black opacity-80 font-black'}`}>
            ...
            <button onClick={() => {setPagina('instellingen'); setGeselecteerdRecept(null);}} className={`flex flex-col items-center gap-1 group w-20 py-2 italic font-black ${pagina === 'instellingen' ? 'text-blue-700' : 'text-black opacity-80 font-black'}`}>
